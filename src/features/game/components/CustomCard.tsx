...
export function CustomCard(props: CustomCardProps) {
  ...
  const { categoryName, setCategoryName } = useCategoryName(
    props.type === 'category' ? props.categoryTitle : '',
  )
  ...
  if (props.type === 'category') {
        ...
        <TooltipTrigger className="h-full w-full">
          <Popover>
            <PopoverTrigger className="h-full w-full">
              <Card
                className={`${categoryColor} cursor-pointer hover:brightness-95 h-full w-full flex justify-center p-1 leading-4`}
              >
                <CardContent>{categoryName}</CardContent>
              </Card>
            </PopoverTrigger>
                ...
                <Textarea
                  ...
                  onBlur={() => {
                    if (!categoryName.trim())
                      setCategoryName(defaultCategoryTitle)
                  }}
                />
              </span>
    ...
    content = (
      <Card
        size="sm"
        className="cursor-pointer hover:brightness-95 grow text-center"
      >
        <CardContent>{props.articleTitle}</CardContent>
      </Card>
    )
  } else {
    content = (
      <Tooltip>
        ...
        <TooltipContent>Add another Category card</TooltipContent>
      </Tooltip>
    )
  }
